...
#               2013-2022 Behan Webster <behanw@converseincode.com>
...
#
# Version 8.41: 2022-12-20 ready-for.sh
#   * Add timeout for checking time with worldtimeapi
#   * Add maximum retries to uses of wget and curl
#
#===============================================================================
VERSION=8.41
...
#   DISTROS[LFXXXX]="Fedora-36+ CentOS-8+ Ubuntu-22.04+"
...
usage() {
    ...
    echo "      --build-dir            Location of directory used for labs in the course"
    ...
    echo "      --no-check             Don't check for HW requirements."
    ...
    if [[ -n $MORE_HELP ]] ; then
        ...
        echo "      --run-as-root          Allow checking packages as root (doesn't download anything)"
        ...
        echo "  -l  --packages PKG CLASS   List a package for all distros in CLASS"
...
while [[ $# -gt 0 ]] ; do
    case "$1" in
        ...
        -r|--remove) REMOVE=y ;;
        --run-as-root) RUNASROOT=y; NOCHECK=y; NOCM=y; NOEXTRAS=y; NOVM=y ;;
        --progress) PROGRESS=y ;;
...
cache_output() {
    ...
    if [[ -s $OUTPUT ]] ; then
        if ! grep -q -E 'FAIL|WARN' "$OUTPUT" ; then
            cat "$OUTPUT"
            return 0
        fi
    fi
...
CURL_TIMEOUT="--location --connect-timeout 10 --retry 3"

#===============================================================================
# Download file (try wget, then curl, then perl)
get() {
    local URL=$1 WGET_OPTS=${2:-} CURL_OPTS=${3:-}
    local WGET="wget --quiet --no-cache --no-check-certificate --prefer-family=IPv4 --timeout=3 --tries=3"
    if [[ -n $NONETWORK ]] ; then
        warn "get: Can't access $URL because no-network selected."
        return 1
    elif [[ -z $USECURL ]] && command -v wget >/dev/null ; then
        debug 2 "  get: $WGET $WGET_TIMEOUT $WGET_OPTS $URL"
        # shellcheck disable=SC2086
        $DRYRUN $WGET $WGET_TIMEOUT $WGET_OPTS "$URL" -O- 2>/dev/null || return 1
...
check_time() {
    ...
    if [[ -n $NONETWORK ]] ; then
        ...
        OUTSIDE=$(wget --quiet --prefer-family=IPv4 --timeout=3 --tries=3 -O- "http://worldtimeapi.org/api/ip" \
                | sed -e 's/^.*unixtime"://; s/,.*$//')
    elif command -v curl >/dev/null ; then
        OUTSIDE=$(curl -s --max-time 3 --retry 3 "http://worldtimeapi.org/api/ip" | sed -e 's/^.*unixtime"://; s/,.*$//')
    else
...
RETRY=
WGET_CM_OPTS="$WGET_PASS --no-check-certificate --prefer-family=IPv4 --timeout=3 --timestamping --tries=3 $WGET_TIMEOUT --tries=3"
CURL_CM_OPTS="$CURL_PASS $CURL_TIMEOUT --retry 3"
...
try_file() {
    ...
    for URL in $(get_urls "$ACTIVITY${FILE:+/$FILE}") ; do
        ...
        # shellcheck disable=SC2086
        SIZE=$(wget --server-response --spider --quiet $WGET_CM_OPTS "$URL" 2>&1 \
                | awk '/Content-Length:/ {print $2}' | tail -1) && break
    done
...
get_activity_file() {
    ...
    FILE=$(get_file "$ACTIVITY" \
        ...
        | sed -Ee 's/^(.*V)([0-9.a-z-]+)(_.*)$/\2.0.0 \1\2\3/' \
...
PACKAGES[LinuxMint-20.2]=
PACKAGES[LinuxMint-20.3]=
#PACKAGES[LinuxMint-21]=
#-------------------------------------------------------------------------------
PACKAGES[openSUSE]=
...
FALLBACK=(
    ...
    [AlmaLinux-8]="CentOS-8 CentOS RHEL-8 RHEL"
    [AlmaLinux-9]="AlmaLinux-8 CentOS-9 CentOS RHEL-9 RHEL"
    ...
    [CentOS.Stream-8]="CentOS-8 CentOS.Stream CentOS RHEL-8 RHEL"
    [CentOS.Stream-9]="CentOS.Stream-8 CentOS.Stream CentOS RHEL"
    [Debian]="Ubuntu"
    [Debian-10]="Ubuntu-20.04 Debian Ubuntu"
    [Debian-11]="Debian-10 Ubuntu-20.04 Debian Ubuntu"
    [Debian-999]="Debian-11 Ubuntu-20.04 Debian Ubuntu"
    [Fedora]="CentOS RHEL"
    [Fedora-35]="Fedora CentOS"
    [Fedora-36]="Fedora-35 CentOS"
    [Fedora-999]="Fedora-35 Fedora"
    [LinuxMint]="Ubuntu"
    [LinuxMint-19.3]="LinuxMint-19.2 Ubuntu-18.04 Ubuntu"
    [LinuxMint-20.2]="LinuxMint-19.3 Ubuntu-20.04 Ubuntu"
    [LinuxMint-20.3]="LinuxMint-20.2 Ubuntu-20.04 Ubuntu"
    #[LinuxMint-21]="LinuxMint-20.3 Ubuntu-22.04 Ubuntu"
    [Mint]="LinuxMint Ubuntu"
    [RHEL]="CentOS"
    [RHEL-7]="CentOS-7 CentOS RHEL"
    [RHEL-8]="CentOS-8 CentOS RHEL"
    [RockyLinux]="CentOS RHEL"
    [RockyLinux-8]="CentOS-8 CentOS RHEL-8 RHEL"
    [RockyLinux-9]="RockyLinux-8 CentOS-9 CentOS RHEL-9 RHEL"
    [openSUSE-15.3]="openSUSE"
    [openSUSE-999]="openSUSE-15.3 openSUSE"
    [SLES]="openSUSE"
    [SLES-15]="openSUSE-15.3 openSUSE"
    [SUSE]="openSUSE"
    [Ubuntu]="Debian"
    [Ubuntu-18.04]="Debian Ubuntu"
    [Ubuntu-20.04]="Debian-10 Debian Ubuntu-18.04 Ubuntu"
    [Ubuntu-22.04]="Ubuntu-20.04 Ubuntu Debian-11 Debian"
...
DISTRO_ALIASES=(
    ...
    [Debian-10-]="Debian-7 Debian-8 Debian-9 Debian-10"
    [Debian-sid]="Debian-999"
    [Fedora-33-]="Fedora-23 Fedora-24 Fedora-25 Fedora-26 Fedora-27 Fedora-28 Fedora-29 Fedora-30 Fedora-31 Fedora-32 Fedora-33"
    ...
    [Ubuntu-LTS]="Ubuntu-18.04 Ubuntu-20.04 Ubuntu-22.04"
    [Ubuntu-18.04+LTS]="Ubuntu-18.04 Ubuntu-20.04 Ubuntu-22.04"
    [Ubuntu-18.04+LTS+]="Ubuntu-18.04 Ubuntu-20.04 Ubuntu-22.04+"
    [Ubuntu-20.04+LTS]="Ubuntu-20.04 Ubuntu-22.04"
    [Ubuntu-20.04+LTS+]="Ubuntu-20.04 Ubuntu-22.04+"
...
DISK=5
#DISTROS=
DISTROS[EMBEDDED]="ArchLinux AlmaLinux-8+ CentOS-8+ Debian-10+ Fedora-35+ LinuxMint-20+ openSUSE-15+ RockyLinux-8+ Ubuntu-20.04+LTS+"
DISTROS[LFD]="ArchLinux AlmaLinux-8+ CentOS-8+ Debian-10+ Fedora-35+ LinuxMint-20+ openSUSE-15+ RHEL-8+ RockyLinux-8+ Ubuntu-18.04+LTS+ SLES-15+"
DISTROS[LFS]="AlmaLinux-8+ CentOS-7+ Debian-10+ Fedora-35+ LinuxMint-20+ openSUSE-15+ RHEL-7+ RockyLinux-8+ Ubuntu-18.04+LTS+ SLES-15+"
DISTRO_ARCH=
DISTRO_BL="CentOS-6- Debian-9- Fedora-33- LinuxMint-19.2- openSUSE-42.3- OracleLinux RHEL-6- SLES-12.2- Ubuntu-21.10-"
DISTRO_BL[EMBEDDED]="CentOS-7- Debian-9- 5 Fedora-34- LinuxMint-19.2- openSUSE-42.3- OracleLinux RHEL SLES Ubuntu-18.04 Ubuntu-21.10-"
...
#-------------------------------------------------------------------------------
PACKAGES[CentOS_@build]="gcc gcc-c++ glibc-devel glibc-utils"
RECOMMENDS[CentOS_@build]="-ccache"
#-------------------------------------------------------------------------------
PACKAGES[openSUSE_@build]="[CentOS_@build] makeinfo -texinfo"
...
PACKAGES[CentOS_@clang]="openssl-devel"

#===============================================================================
# Common packages used in various places
#===============================================================================
PACKAGES[@common]="bzip2 curl dos2unix file gawk git gzip psmisc sudo
	tar tree unzip wget"
RECOMMENDS[@common]="etckeeper rsync zip"
SUGGESTS[@common]="emacs gnome-tweaks gparted nano net-tools open-vm-tools"
#-------------------------------------------------------------------------------
PACKAGES[Ubuntu_@common]="emacs-gtk gnupg gnupg2 iputils-ping screen tofrodos xz-utils"
PACKAGES[Ubuntu-18.04_@common]="-emacs-gtk"
RECOMMENDS[Ubuntu_@common]="aptitude htop synaptic unp"
SUGGESTS[Ubuntu_@common]="vim"
#-------------------------------------------------------------------------------
PACKAGES[CentOS_@common]="gnupg2"
SUGGESTS[CentOS_@common]="vim-enhanced"
SUGGESTS[CentOS_@common]="vim-enhanced"
SUGGESTS[CentOS-7_@common]="[CentOS_@common] -gnome-tweaks gnome-tweak-tool screen"
SUGGESTS[CentOS-8_@common]="[CentOS_@common]"
RECOMMENDS[CentOS_@common]="-etckeeper"
SUGGESTS[CentOS_@common]="-gparted"
PACKAGES[Fedora_@common]="NetworkManager-tui screen xz"
RECOMMENDS[Fedora_@common]="[CentOS_@common]"
#-------------------------------------------------------------------------------
PACKAGES[openSUSE_@common]="gpg2 xz screen"
...
PACKAGES[Debian-10_@docker]="docker.io docker-compose"
PACKAGES[Debian-999_@docker]="docker.io"
PACKAGES[Ubuntu_@docker]="docker.io docker-compose"
PACKAGES[Fedora_@docker]="[CentOS_@docker]"
PACKAGES[openSUSE_@docker]="docker"
PACKAGES[CentOS_@docker]="podman podman-docker"
PACKAGES[CentOS-7_@docker]="-podman -podman-docker docker-common"
PACKAGES[CentOS-8_@docker]="-"
...
# Embedded packages
#===============================================================================
PACKAGES[@embedded]="bc gperf lzop python3-pip"
SUGGESTS[@embedded]="minicom"
#-------------------------------------------------------------------------------
PACKAGES[CentOS_@embedded]="dnsmasq dtc glibc-static libyaml-devel nfs-utils"
PACKAGES[CentOS-7_@embedded]="-dtc screen"
RECOMMENDS[CentOS-7_@embedded]="uboot-tools"
RECOMMENDS[Fedora_@embedded]="bmap-tools uboot-tools screen"
SUGGESTS[Fedora_@embedded]="cutecom"
#-------------------------------------------------------------------------------
PACKAGES[openSUSE_@embedded]="dtc nfs-kernel-server screen"
...
PACKAGES[Ubuntu_@embedded]="dnsmasq-base dosfstools eject libyaml-dev mtd-utils
	nfs-kernel-server parted screen"
...
RECOMMENDS[LinuxMint-20.2_@embedded]="[@Ubuntu_@embedded]"

#===============================================================================
# Golang packages
#===============================================================================
PACKAGES[LFS153]="golang"
PACKAGES[CentOS-7_LFS153]="-golang"
PACKAGES[CentOS-8_LFS153]="-"
PACKAGES[openSUSE_LFS153]="-golang"
...
PACKAGES[CentOS_@java]="java-1.8.0-openjdk"
...
#===============================================================================
PACKAGES[@kernel]="@build bc crash cscope gitk sysfsutils"
#-------------------------------------------------------------------------------
PACKAGES[Debian_@kernel]="exuberant-ctags indent libelf-dev libfuse-dev libhugetlbfs-dev
	libncurses-dev libssl-dev liblzo2-dev nfs-common sparse swig"
RECOMMENDS[Debian_@kernel]="symlinks"
...
#-------------------------------------------------------------------------------
PACKAGES[CentOS_@kernel]="ctags elfutils-libelf-devel fuse-devel kernel-devel
	ncurses-devel openssl-devel"
PACKAGES[CentOS-7_@kernel]="indent"
PACKAGES[AlmaLinux-9_@kernel]="-ctags"
PACKAGES[CentOS.Stream-9_@kernel]="-ctags"
PACKAGES[RockyLinux-9_@kernel]="-ctags"
RECOMMENDS[CentOS_@kernel]="libhugetlbfs-devel nfs-utils"
RECOMMENDS[CentOS-8_@kernel]="qt5-devel"
RECOMMENDS[CentOS.Stream_@kernel]="-sparse"
RECOMMENDS[AlmaLinux-9_@kernel]="-libhugetlbfs-devel"
RECOMMENDS[CentOS.Stream-9_@kernel]="-libhugetlbfs-devel"
RECOMMENDS[RockyLinux-9_@kernel]="-libhugetlbfs-devel"
RECOMMENDS[Fedora_@kernel]="[CentOS_@kernel] -libhugetlbfs-devel"
SUGGESTS[CentOS_@kernel]="gtk2-devel slang-devel"
SUGGESTS[CentOS-7_@kernel]="libglade2-devel"
SUGGESTS[CentOS-8_@kernel]="qt-devel"
SUGGESTS[CentOS-8_@kernel]="-libglade2-devel"
SUGGESTS[CentOS.Stream_@kernel]="-libglade2-devel -qt-devel"
SUGGESTS[RockyLinux-9_@kernel]="-libglade2-devel"
SUGGESTS[Fedora_@kernel]="[CentOS_@kernel] qt-devel"
#-------------------------------------------------------------------------------
PACKAGES[openSUSE_@kernel]="ctags fuse-devel kernel-devel libelf-devel lzo-devel
	libopenssl-devel ncurses-devel"
RECOMMENDS[openSUSE_@kernel]="libhugetlbfs"
SUGGESTS[openSUSE_@kernel]="gtk2-devel libglade2-devel slang-devel"
RECOMMENDS[openSUSE-999_@kernel]="libhugetlbfs-devel"

#===============================================================================
# Kernel Trace/perf related packages
#===============================================================================
PACKAGES[@ktrace]="kernelshark trace-cmd"
PACKAGES[Debian_@ktrace]="linux-perf"
PACKAGES[Ubuntu_@ktrace]="-linux-perf linux-tools-generic"
PACKAGES[CentOS_@ktrace]="perf"
PACKAGES[CentOS-7_@ktrace]="-kernelshark"
PACKAGES[Fedora_@ktrace]="perf"
PACKAGES[openSUSE_@ktrace]="perf"
...
#-------------------------------------------------------------------------------
PACKAGES[CentOS_@perf]="binutils-devel elfutils-libelf-devel
	slang-devel openssl-devel libzstd-devel zlib-devel"
PACKAGES[CentOS-7_@perf]="libdwarf-devel libunwind-devel -libzstd-devel"
PACKAGES[CentOS-8_@perf]="libdwarf-devel"
RECOMMENDS[CentOS_@perf]="libaio-devel perl-devel"
RECOMMENDS[CentOS-8_@perf]="platform-python-devel"
SUGGESTS[CentOS_@perf]="gtk2-devel"
PACKAGES[Fedora_@perf]="[CentOS_@perf] libdwarf-devel"
#-------------------------------------------------------------------------------
PACKAGES[openSUSE_@perf]="binutils-devel audit-devel libdw-devel libdwarf-devel
	libelf-devel slang-devel libopenssl-devel libunwind-devel libzstd-devel
	zlib-devel"
...
PACKAGES[LinuxMint-20.2_@qemu]="libglib2.0-dev libpixman-1-dev"
PACKAGES[CentOS_@qemu]="glib2-devel pixman-devel SDL2-devel zlib-devel"
PACKAGES[CentOS-7_@qemu]="-SDL2-devel -ninja-build"
PACKAGES[CentOS-8_@qemu]="platform-python-devel"
...
PACKAGES[CentOS_@stress]="-stress -stress-ng"
PACKAGES[CentOS-7_@stress]="-stress-ng"
...
PACKAGES[CentOS_@sysadm]="[Fedora_@sysadm] -bonnie++ -collectl"
PACKAGES[CentOS.Stream_@sysadm]="[CentOS_@sysadm]"
PACKAGES[CentOS.Stream-9_@sysadm]="-gnuplot"
PACKAGES[AlmaLinux-9_@sysadm]="-gnuplot"
PACKAGES[RockyLinux-9_@sysadm]="-gnuplot"
PACKAGES[Ubuntu_@sysadm]="iftop"
PACKAGES[openSUSE_@sysadm]="bonnie termcap"

#===============================================================================
# Userspace Trace/perf related packages
#===============================================================================
...
#===============================================================================
PACKAGES[@virt]="virt-manager virt-viewer"
PACKAGES[Debian_@virt]="bridge-utils ebtables libosinfo-bin libvirt-daemon-system \
	libvirt-clients qemu-utils spice-client-gtk virtinst"
PACKAGES[Debian-10_@virt]="qemu-kvm"
PACKAGES[Debian-11_@virt]="qemu-system-x86 ovmf"
PACKAGES[Ubuntu_@virt]="[Debian_@virt]"
PACKAGES[Ubuntu-18.04_@virt]="libvirt-bin qemu-kvm -qemu-system-x86"
PACKAGES[CentOS_@virt]="edk2-ovmf spice-gtk spice-gtk-tools"
PACKAGES[CentOS.Stream_@virt]="[CentOS_@virt] -bridge-utils -ebtables"
PACKAGES[CentOS-7_@virt]="bridge-utils ebtables -edk2-ovmf -spice-gtk"
PACKAGES[CentOS-8_@virt]="-"
PACKAGES[AlmaLinux-9_@virt]="-spice-gtk -spice-gtk-tools"
PACKAGES[CentOS.Stream-8_@virt]="-"
PACKAGES[CentOS.Stream-9_@virt]="-spice-gtk -spice-gtk-tools"
PACKAGES[RockyLinux-9_@virt]="-spice-gtk -spice-gtk-tools"
PACKAGES[Fedora_@virt]="bridge-utils spice-gtk"
PACKAGES[openSUSE_@virt]="bridge-utils ebtables libvirt-daemon qemu-kvm spice-gtk -spice-gtk-tools"
...
#===============================================================================
# Extra requirements for LFD112
#===============================================================================
TITLE[LFD112]="Building Applications with RISC-V and FreeRTOS"
WEBPAGE[LFD112]="$LFDTRAINING/building-applications-with-risc-v-and-freertos-lfd112x"
INPERSON[LFD112]=n
VIRTUAL[LFD112]=n
SELFPACED[LFD112]=y
MOOC[LFD112]=edX
OS[LFD112]="$ANYOS"
INTERNET[LFD112]="$REQUIRED"
...
PACKAGES[LFD301]="@build @common @java @stress @sysadm @trace 
	cvs lvm2 subversion sysstat tcpdump wireshark"
...
PACKAGES[Debian-999_LFD301]="-ksysguard"
PACKAGES[Ubuntu-22.04_LFD301]="-ksysguard"
PACKAGES[CentOS_LFD301]="-cvs git-all expat-devel openssl-devel"
#PACKAGES[CentOS.Stream_LFD301]="-cvs"
RECOMMENDS[CentOS-6_LFD301]="-iptraf-ng iptraf kdebase -ksysguard ksysguardd"
RECOMMENDS[CentOS.Stream_LFD301]="-ksysguard"
PACKAGES[Fedora_LFD301]="[CentOS_LFD301]"
RECOMMENDS[Fedora_LFD301]="[CentOS_LFD301]"
RECOMMENDS[openSUSE_LFD301]="-ksysguard"
RECOMMENDS[openSUSE-15.3]="kdebase4-workspace"
...
PACKAGES[AlmaLinux_LFD401]="-ddd"
PACKAGES[RockyLinux_LFD401]="-ddd"
PACKAGES[CentOS_LFD401]="-ddd glibc-static libaio-devel"
PACKAGES[CentOS-7_LFD401]="ElectricFence"
PACKAGES[CentOS-8_LFD401]="-"
PACKAGES[CentOS.Stream_LFD401]="[CentOS_LFD401] -ElectricFence"
PACKAGES[Fedora_LFD401]="ElectricFence glibc-static kcachegrind libaio-devel"
PACKAGES[openSUSE_LFD401]="kcachegrind libaio-devel"
...
#-------------------------------------------------------------------------------
PACKAGES[LFD420]="@common @kernel @perf @stress @ktrace @trace"
...
#-------------------------------------------------------------------------------
PACKAGES[LFD450]="@build @common @embedded @kernel @qemu @qemuriscv @ktrace @trace
	cmake help2man rsync"
...
#-------------------------------------------------------------------------------
PACKAGES[LFD460]="@build @common chrpath cpio diffstat
	python3-jinja2 python3-pexpect python3-pip python3-ply socat texinfo xterm"
#-------------------------------------------------------------------------------
PACKAGES[Ubuntu_LFD460]="liblz4-tool libegl1-mesa libsdl2-dev mesa-common-dev
	pylint python3 python3-git python3-subunit zstd"
PACKAGES[Ubuntu-18.04_LFD460]="-pylint pylint3"
PACKAGES[Ubuntu-20.04_LFD460]="-"
PACKAGES[CentOS_LFD460]="cpp diffutils libarchive-devel mesa-libGL-devel
	perl-Data-Dumper perl-Text-ParseWords perl-Thread-Queue
	rpcgen perl tar SDL2-devel sqlite which xz"
PACKAGES[CentOS-7_LFD460]="-python3-jinja2 -python3-pexpect -python3-ply -rpcgen -SDL2-devel"
PACKAGES[CentOS-8_LFD460]="-python3-GitPython"
...
PACKAGES[openSUSE_LFD460]="Mesa-dri-devel
	Mesa-libEGL1 rpcgen python3 python3-curses -python3-jinja2 python3-Jinja2
	python3-curses which"
PACKAGES[openSUSE-999_LFD460]="python -python3 -python3-curses
...
PACKAGES[CentOS.Stream-9_LFS101]="-qpdf"
PACKAGES[RockyLinux-9_LFS101]="-qpdf"
...
PACKAGES[Fedora_LFS153]="community-mysql-server -mysql-server"
PACKAGES[CentOS_LFS153]="mysql-server"
PACKAGES[CentOS-7_LFS153]="-mysql-server -nodejs"
PACKAGES[CentOS-8_LFS153]="-"
PACKAGES[openSUSE_LFS153]="-nodejs -php"
#PACKAGES[openSUSE-15.3_LFS153]="nodejs12"
...
PACKAGES[Fedora_LFS176]="libsodium"
...
#===============================================================================
# Extra requirements for LFS182
#===============================================================================
TITLE[LFS182]="Securing Your Software Supply Chain With Sigstore"
WEBPAGE[LFS182]="$LFSTRAINING/securing-your-software-supply-chain-with-sigstore-lfs182x"
INPERSON[LFS182]=n
VIRTUAL[LFS182]=n
SELFPACED[LFS182]=y
MOOC[LFS182]=edX
ARCH[LFS182]="x86_64 aarch64"
RAM[LFS182]="2"
OS[LFS182]="$UNIXOS"
INTERNET[LFS182]="$REQUIRED"
PACKAGES[LFS182]="@common @docker @golang"

#===============================================================================
# Extra requirements for LFS200
#===============================================================================
TITLE[LFS200]="Fundamentals of Open Source IT and Cloud Computing"
WEBPAGE[LFS200]="$LFSTRAINING/fundamentals-of-open-source-it-and-cloud-computing-lfs200"
INPERSON[LFS200]=n
VIRTUAL[LFS200]=n
SELFPACED[LFS200]=y
MOOC[LFS200]=edX
INTERNET[LFS200]="$REQUIRED"

#===============================================================================
# Extra requirements for LFS207
#===============================================================================
TITLE[LFS207]="Linux System Administration Essentials"
WEBPAGE[LFS207]="$LFSTRAINING/linux-system-administration-essentials-lfs207"
INPERSON[LFS207]=n
VIRTUAL[LFS207]=n
SELFPACED[LFS207]=y
INSTR_LED_CLASS[LFS207]="LFS307"
COPYPACKAGES[LFS207]="LFS211"
...
PACKAGES[Ubuntu_LFS211]="bsd-mailx dovecot-imapd dovecot-lmtpd dovecot-pop3d
	openssh-server"
RECOMMENDS[Ubuntu-18.04_LFS211]="zenmap"
RECOMMENDS[Ubuntu-20.04_LFS211]="-zenmap"
RECOMMENDS[Debian-11_LFS211]="-zenmap"
PACKAGES[CentOS_LFS211]="dovecot kernel-modules-extra mailx openssh-server"
PACKAGES[CentOS-7_LFS211]="-kernel-modules-extra"
PACKAGES[AlmaLinux-9_LFS211]="-mailx"
PACKAGES[CentOS.Stream-9_LFS211]="-mailx"
PACKAGES[RockyLinux-9_LFS211]="-mailx"
...
PACKAGES[CentOS_LFS216]="kernel-devel"
PACKAGES[openSUSE_LFS216]="[CentOS_LFS216]"
...
#===============================================================================
TITLE[LFS264]="Anuket Fundamentals"
WEBPAGE[LFS264]="$LFSTRAINING/anuket-fundamentals"
INPERSON[LFS264]=n
VIRTUAL[LFS264]=n
SELFPACED[LFS264]=y
...
PACKAGES[CentOS.Stream-9_LFS300]="-qpdf"
PACKAGES[RockyLinux-9_LFS300]="-qpdf"

#===============================================================================
# Extra requirements for LFS303
#===============================================================================
TITLE[LFS303]="Linux for Cloud Technicians"
WEBPAGE[LFS303]="$LFSTRAINING/linux-for-cloud-technicians-lfs303"
DISK[LFS303]="1"
#-------------------------------------------------------------------------------
COPYPACKAGES[LFS303]="LFS211"

#===============================================================================
# Extra requirements for LFS307
#===============================================================================
TITLE[LFS307]="Linux System Administration"
WEBPAGE[LFS307]="$LFSTRAINING/linux-system-administration-lfs307"
DISK[LFS307]="1"
SELFPACED_CLASS[LFS307]="LFS207"
#-------------------------------------------------------------------------------
COPYPACKAGES[LFS307]="LFS211"
...
PACKAGES[CentOS_LFS462]="kernel-devel"
#PACKAGES[CentOS-7_LFS462]="-firewalld"
PACKAGES[openSUSE_LFS462]="[CentOS_LFS462]"
...
PACKAGES[AlmaLinux-9_LFW111]="-python2"
PACKAGES[CentOS.Stream-9_LFW111]="-python2"
PACKAGES[RockyLinux-9_LFW111]="-python2"
    ...
    for A in $ACTIVITIES; do
        ...
        for DIST in $DISTS; do
            ...
            if [[ -n $P ]] ; then
                ...
                OPTS+=" -df $DIST"
                # shellcheck disable=SC2086
                debug 2 "    check_packages: $SEARCH $OPTS "$P
                # shellcheck disable=SC2086
                OUTPUT=$($SEARCH $OPTS $P)
                if [[ -n $OUTPUT ]] ; then
                    [[ -n $VERBOSE ]] || echo
...
#===============================================================================
# TEST: Enough free disk space in $BUILDDIR? (defaults to $HOME)
check_free_disk() {
    local DSK=$1
    local BUILDDIR=$2
    [[ -n $BUILDDIR ]] || BUILDDIR=$(getent passwd "$USER" | cut -d: -f6)
    [[ -n $BUILDDIR ]] || error "No BUILDDIR specified"
    verbose "check_free_disk: DSK=$DSK BUILDDIR=$BUILDDIR"
    local DISK_GBYTES ALT

    DISK_GBYTES=$(get_df "$BUILDDIR" GiB)
    if [[ -z $DISK_GBYTES ]] ; then
        bug "I didn't find how much disk space is free in $BUILDDIR" \
            "df --output=avail $BUILDDIR | awk '{if (NR == 2) print int(($4+524288)/1048576)}'"
    elif [[ ${DISK_GBYTES:=1} -lt $DSK || -n $SIMULATE_FAILURE ]] ; then
        ALT=$(find_alternate_disk "$DSK" GiB "    --build-dir ")
        if [[ -n $ALT ]] ; then
            warn "$BUILDDIR only has $DISK_GBYTES GiB free (need at least $DSK GiB)"
            pass "However, there is space on other drives. Consider building labs in one of these:\n$ALT"
        else
            fail "only $DISK_GBYTES GiB free in $BUILDDIR (need at least $DSK GiB)
        If you plan to use another mounted disk for this course, please provide the path
        using '--build-dir /path/to/dir' to indicate where that is."
        fi
    else
...
check_all() {
    ...
    check_ram "$RAM"
    check_free_disk "$DISK" "${BUILDDIR:-$HOME}"
